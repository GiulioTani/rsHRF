    ...
    def __init__(self):
        ...
        def retrieveHRF(get_pos=False):
            if "Preprocessed-BOLD" not in current.get():
                logger.putLog(
                    status=Status(
                        False,
                        error="Select a Preprocessed-BOLD Timeseries to retrieve HRF",
                    )
                )
            else:
                start = time.process_time()
        ...
        def retrieveHRFParameters(pos=None):
            ...
            if "HRF" not in current.get():
                logger.putLog(
                    status=Status(
                        False, error="Select an HRF Timeseries to retrieve parameters"
                    )
                )
            else:
                start = time.process_time()
        ...
        def deconvolveHRF(pos=None):
            ...
            if "HRF" not in current.get():
                logger.putLog(
                    status=Status(
                        False, error="Select an HRF Timeseries to deconvolve BOLD"
                    )
                )
            else:
                start = time.process_time()
        ...
        def runCompletePipeline():
            if "Preprocessed-BOLD" in current.get():
                ...
                logger.putLog(
                    status=Status(
                        False,
                        error="Select an HRF or Preprocessed-BOLD Timeseries to run pipeline",
                    )
                )
            ...
            try:
                if len(current.get().split("_")) == 3:
                    ...
                    if core.save_info(current.get(), out):
                        logger.putLog(
                            status=Status(True, info="File saved successfully")
                        )
                    else:
            ...
            try:
                ...
                if log:
                    logger.putLog(
                        status=Status(
                            True,
                            info="Updated values for subject: " + current_subject.get(),
                        )
                    )
                temp.append("None")
        ...
        changeParameterButton = Button(
            root, text="Update Parameters", command=updateParameters, height=1, width=20
        )
        retrieveHRFButton = Button(
            root, text="Retrieve HRF", command=retrieveHRF, height=1, width=20
        )
        retrieveHRFParametersButton = Button(
            root,
        ...
        deconvolveHRFButton = Button(
            root, text="Deconvolve BOLD", command=deconvolveHRF, height=1, width=20
        )
        updatePlotsButton = Button(
            root, text="Update Plots", command=updatePlots, height=1, width=20
        )
        changeEstimationRule = Button(
            root,
        ...
        getValueInfo = Button(
            root, text="Get Info", command=getInfo, height=1, width=20
        )
        storeValue = Button(
            root, text="Save Value", command=saveValue, height=1, width=20
        )
        updateStore = Button(
            root, text="Update Values", command=updateValues, height=1, width=20
        )
        runCompletePipeline = Button(
            root, text="Run Pipeline", command=runCompletePipeline, height=1, width=20
        )
        saveAllValues = Button(
            root, text="Save All", command=saveForSubject, height=1, width=20
        )
