...
class PlotterOptionsWindow:
    def __init__(self):
        ...
        window.geometry(
            "650x200+%d+%d" % (screen_width / 2.95, 100 + screen_height / 2)
        )
                    ...
                    for each in self.plotables:
                        if each[0] == self.plot[i].get():
                            self.plotterScreen.makePlot(
                                each[1][:, int(self.selectVoxel[i].get())], 1, i
                            )
                            return
                    self.plotterScreen.makePlot(None, 0, i)
                    return
    ...
    def updateWidgets(self):
        ...
        for choice in self.options:
            for i in range(0, self.numberOfPlots):
                self.plotSelectDropDown[i]["menu"].add_command(
                    label=choice, command=_setit(self.plot[i], choice)
                )
