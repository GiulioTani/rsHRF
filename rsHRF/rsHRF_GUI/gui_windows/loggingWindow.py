    ...
    def putLog(self, plotables=None, data_info=None, status=None):
        ...
        if status is not None:
            if status.get_state():
                ...
                self.text.insert(
                    INSERT, ">> " + current_time + "\t SUCCESS " + message + "\n"
                )
                self.text.tag_add(
                    "success", str(self.lineNum) + ".12", str(self.lineNum) + ".20"
                )
                self.text.tag_config("success", foreground="green")
                self.text.tag_add(
                    "success message",
                    str(self.lineNum) + ".20",
                    str(self.lineNum) + "." + str(21 + len(message)),
                )
                self.text.tag_config("success message", foreground="LightBlue1")
            else:
                message = status.get_error()
                self.text.insert(
                    INSERT, ">> " + current_time + "\t ERROR " + message + "\n"
                )
                self.text.tag_add(
                    "error", str(self.lineNum) + ".12", str(self.lineNum) + ".18"
                )
                self.text.tag_config("error", foreground="red")
                self.text.tag_add(
                    "error message",
                    str(self.lineNum) + ".18",
                    str(self.lineNum) + "." + str(19 + len(message)),
                )
                self.text.tag_config("error message", foreground="yellow")
            self.text.tag_add(
                "time", str(self.lineNum) + ".03", str(self.lineNum) + ".11"
            )
            ...
            for each in dic.keys():
                self.text.insert(
                    INSERT, ">> " + current_time + "\t" + each + str(dic[each]) + "\n"
                )
                self.text.tag_add(
                    "time", str(self.lineNum) + ".03", str(self.lineNum) + ".11"
                )
                ...
                self.text.tag_add(
                    "dic_label",
                    str(self.lineNum) + ".12",
                    str(self.lineNum) + "." + str(12 + len(each)),
                )
            ...
            self.text.tag_add(
                "labels", str(self.lineNum) + ".12", str(self.lineNum) + "." + str(37)
            )
            self.text.tag_config("labels", foreground="PaleTurquoise1")
            self.text.tag_add(
                "titles",
                str(self.lineNum + 2) + ".00",
                str(self.lineNum + 2) + "." + str(37),
            )
            ...
            self.text.insert(INSERT, ">> " + current_time + "\t Information \n\n")
            self.text.tag_add(
                "labels", str(self.lineNum) + ".12", str(self.lineNum) + "." + str(37)
            )
            self.text.tag_config("labels", foreground="PaleTurquoise1")
            ...
            if data_info["Type"] == "BOLD":
                ...
                self.text.insert(
                    ...
                    + "   :- "
                    + "Associated HRF: "
                    + data_info["Associated HRF"]
                    + "\n",
                )
            self.text.tag_config("type", foreground="azure")
            self.text.tag_config("line2", foreground="azure")
            self.lineNum += 2
            self.text.insert(INSERT, "   :- " + "\t\t\tAssociated Parameters\n")
            self.text.tag_add(
                "ap", str(self.lineNum) + ".03", str(self.lineNum) + ".30"
            )
            ...
            for each in data_info["Parameters"].keys():
                self.text.insert(
                    ...
                    + each
                    + "\t\t\t=\t\t"
                    + str(data_info["Parameters"][each])
                    + "\n",
                )
                self.text.tag_add(
                    "pv",
                    str(self.lineNum) + ".00",
                    str(self.lineNum) + "." + str(6 + len(each)),
                )
