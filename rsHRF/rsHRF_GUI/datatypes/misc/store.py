...
class Store:
    ...
    def get_info(self, s):
        ...
        if ts.get_label() == "BOLD":
            ...
            dic["Associated Raw BOLD"] = (
                subject_index
                + "_BOLD_"
                + subject.get_time_series_pos(ts.get_BOLD_Raw())
            )
        elif ts.get_label() == "HRF":
            dic["Associated BOLD"] = (
                subject_index
                + "_Preprocessed-BOLD_"
                + subject.get_time_series_pos(ts.get_associated_BOLD())
            )
        elif ts.get_label() == "Deconvolved-BOLD":
            dic["Associated HRF"] = (
                subject_index
                + "_HRF_"
                + subject.get_time_series_pos(ts.get_associated_HRF())
            )
        return dic
